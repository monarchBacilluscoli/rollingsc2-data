...
print(whole.dtypes)
whole[("Damage")] = whole["Damage"].astype(np.float)
whole[("Hurt")] = whole["Hurt"].astype(np.float)
...
# sns.pairplot(data=whole[(['Hurt', 'Damage', 'Settings'])],hue="Settings", height=2, aspect=1.2)
# sns.pointplot(data=whole, x="Hurt", y="Damage",
#   hue="Settings", height=2, aspect=1.2)
sns.jointplot(data=whole, x="Hurt", y="Damage",
              hue="Settings", height=4)
plt.savefig("2_3_dis.svg")
plt.show()
# todo 同样配置的数据用字典怼一起去.
